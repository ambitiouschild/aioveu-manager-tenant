    ...
    <view class="component-container__main">
      ...
      <view class="spec-card">
        ...
        <view class="spec-form" v-if="specForm.specList.length > 0">
          <view v-for="(spec, specIndex) in specForm.specList" :key="spec.id" class="spec-item">
            <!-- 规格名称 -->
              ...
              <view v-if="specIndex === 0 && spec.values.length > 0" class="spec-image-section">
                <view class="image-upload-area" @tap="uploadSpecImage(specIndex)">
                  <view v-if="getFirstSpecImage(spec)" class="image-preview">
                    <image :src="getFirstSpecImage(spec)" mode="aspectFill" class="spec-image" />
                ...
                <view class="upload-tip">最大图片大小：5MB，支持格式：JPG、JPEG、PNG</view>
              </view>
            </view>
        ...
        <view v-else class="empty-state">
          <image src="/static/empty-spec.png" class="empty-image" mode="aspectFit" />
      ...
      <!-- SKU表格改为卡片式布局 -->
      <view class="stock-card" v-if="hasValidSpecs">
        <view class="card-header">
          <text class="card-title">商品库存</text>
          <text class="sku-count">共 {{ skuForm.skuList.length }} 个SKU</text>
        </view>
        ...
        <view class="sku-list" v-if="skuForm.skuList.length > 0">
          <view v-for="(sku, skuIndex) in skuForm.skuList" :key="skuIndex" class="sku-card">
            <!-- 在SKU卡片开头添加 -->
            ...
            <view class="sku-card-header">
              <text class="sku-specs">
                <text v-for="i in specTitles.length" :key="i" class="spec-item">
              ...
              <view class="form-item">
                ...
                <view class="image-upload-area" @tap="uploadSkuImage(skuIndex)">
                  <view v-if="sku.picUrl" class="image-preview">
                    <image :src="sku.picUrl" mode="aspectFill" class="uploaded-image" />
                    <view class="image-actions">
                      <view class="btn-action replace" @tap.stop="uploadSkuImage(skuIndex)">
                        更换
                      </view>
                      <view class="btn-action delete" @tap.stop="removeSkuImage(skuIndex)">
                        删除
                      </view>
                    </view>
                  </view>
        ...
        <!-- SKU空状态 -->
        <view v-else class="empty-state">
          <image src="/static/empty-sku.png" class="empty-image" mode="aspectFit" />
          <text class="empty-text">请先配置规格值</text>
        </view>
      </view>
    </view>
...
const generateSkuList = () => {
  ...
  console.log(
    `✅ 生成 ${newSkus.length} 个SKU，其中${newSkus.filter((s) => s.id).length}个是已有的`
  );
};
...
const uploadSkuImage = async (skuIndex: number) => {
  try {
    ...
    // 检查是否是用户取消操作
    const isUserCancel =
      error.message && (error.message.includes("cancel") || error.message.includes("取消"));
      ...
      .spec-item {
        ...
        .spec-values-section {
          ...
          .spec-tags {
            ...
            /* 核心样式：胶囊标签 (Pill Shape) */
            .spec-tag {
              display: flex;
              align-items: center;
              padding: 12rpx 30rpx; /* 右边留出空间给删除按钮 */
              border: 2rpx solid;
              border-radius: 8rpx; /* 关键：实现胶囊效果 */
              background-color: #ffffff;
